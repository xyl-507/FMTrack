...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        for frame_num, frame_path in enumerate(zip(seq.frames[0][1:], seq.frames[1][1:]), start=1):
            ...
            if len(seq.ground_truth_rect) > 1 and 'vtuav' not in seq.dataset:  # multi GT + vtuav是隔10帧标注，GT和帧数对不上，不加载gt，用于可视化的，xyl
                info['gt_bbox'] = seq.ground_truth_rect[frame_num]
            out = tracker.track(image, info)
            # out = tracker.track(image, frame_num,info)  # xyl for heatmap
            prev_output = OrderedDict(out)
