...
class ODTrack(BaseTracker):
    ...
    def initialize(self, image, info: dict):
        ...
        with torch.no_grad():
            ...
            # self.memory_frames = [template.tensors[:,:3,:,:], template.tensors[:,3:,:,:]]
            self.memory_frames.append([template.tensors[:,:3,:,:], template.tensors[:,3:,:,:]])
    ...
    # def track(self, image, frame_num, info: dict = None):  # xyl for heatmap
        H, W, _ = image.shape
        self.frame_id += 1
        ...
        with torch.no_grad():
            out_dict = self.network.forward(template=template_list, search=[[search.tensors[:,:3,:,:],search.tensors[:,3:,:,:]]], ce_template_mask=box_mask_z)
        ...
        vis_xyl = False
        if vis_xyl:
            print('=================== 请注意，lib/test/tracker/ostrack.py line 141 正在可视化 image {} 热力图 ============'.format(frame_num))
            ...
            img1 = x_patch_arr[:,:,:3].squeeze()  # 用 x_patch_arr 作为原图，RGBT中是(256, 256, 6)
            ...
            path = '/public/workspace/xyl/ODTrack-RGBT/output/vis'
            ...
            if not os.path.exists(path_heatmap):
                os.mkdir(path_heatmap)  # os.mkdir()创建路径中的最后一级目录，而os.makedirs()创建多层目录
            ...
            cv2.imwrite('{}/heatmap_{:06d}.jpg'.format(path_heatmap, frame_num), heatmap)
            cv2.imwrite('{}/img_orign_{:06d}.jpg'.format(path_img_orign, frame_num), img1)
            cv2.imwrite('{}/heatmap+img_{:06d}.jpg'.format(path_heatmap_img, frame_num), heatmap_sum)
    ######################################################################################################## Heatmap xyl
        ...
        frame = [cur_frame.tensors[:,:3,:,:], cur_frame.tensors[:,3:,:,:]]
        # mask = cur_frame.mask
        
        # if self.frame_id > self.cfg.TEST.MEMORY_THRESHOLD:
        #     frame[0] = frame[0].detach().cpu()
        #     frame[1] = frame[1].detach().cpu()
            # mask = mask.detach().cpu()
        self.memory_frames.append(frame)
        if self.cfg.MODEL.BACKBONE.CE_LOC:  # use CE module
            template_bbox = self.transform_bbox_to_crop(self.state, z_resize_factor, frame[0].device).squeeze(1) # template_bbox = self.transform_bbox_to_crop(self.state, z_resize_factor, frame.device).squeeze(1)
            self.memory_masks.append(generate_mask_cond(self.cfg, 1, frame[0].device, template_bbox)) # self.memory_masks.append(generate_mask_cond(self.cfg, 1, frame.device, template_bbox))
