...
import sys
import sys
sys.path.append("/public/zhongbineng/workspace/lxh/envs/ostrack_mamba/lib/python3.9/site-packages")
...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='odtrack', choices=['odtrack'],
                        help='training script name')
    parser.add_argument('--config', type=str, default='baseline_256_lasher_mamba_v6', help='yaml configure file name')
    args = parser.parse_args()

    return args


def evaluate_odtrack(model, template, search, ce_template_mask):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search, ce_template_mask),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search, ce_template_mask)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search, ce_template_mask)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if args.script == "odtrack":
        ...
        search_list.append([search,search])
        template_len = 3
        ...
        # ce
        for i in range(template_len):
            ce_z = torch.randn(bs, template_len*64)
            ce_z = ce_z.to(device)
            box_mask_z.append(ce_z)
        ce_template_mask = torch.cat(box_mask_z, dim=1)    
        
            
        merge_layer = cfg.MODEL.BACKBONE.MERGE_LAYER
        if merge_layer <= 0:
            evaluate_odtrack(model, template_list, search_list, ce_template_mask)
        else:
            evaluate_vit_separate(model, template, search, ce_template_mask)
